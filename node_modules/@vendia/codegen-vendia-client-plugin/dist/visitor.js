...
const jsonSchemaToTypescriptType = {
    string: 'string',
    integer: 'number',
    number: 'number',
    boolean: 'boolean',
...
export class GraphQLRequestVisitor extends ClientSideBaseVisitor {
    ...
    constructor(schema, fragments, rawConfig) {
        ...
        // This fixes an esoteric TS bug (Error...This is likely not portable. A type annotation is necessary), because DocumentNode is
        // the return type of gql tag and it explicitly importing it here fixes the issue...
...
export function getSharingPolicies({_gqlClient, _subscribe, _requestWrapper = defaultWrapper}: GetSdkNamespaceOptions) {
  return {
${groupedVendiaActions.sharingPolicies}
  };
}

type GetSharingPolicies = ReturnType<typeof getSharingPolicies>
`;
        ...
        function buildAction(o) {
            ...
            const entityJson = this.config.schemaJson?.properties?.[entityName];
            const uniqueKeys = entityJson?.items?.['x-vendia-unique'] ?? [];
            const hasUniqueKeys = uniqueKeys.length > 0 && uniqueKeys.some((key) => entityJson?.items?.properties?.[key] !== undefined);
            ...
            if (o.operationType === 'Query') {
                // product.get(id, options) OR product.get(options) - both APIs are supported.
                // For entities with unique keys enabled, must use the options object to pass in the unique key value(s) - id string as first arg
                //  will be assumed to always be the vendia _id field since we can't guess which unique key field(s) to use.
                // Same goes for delete/remove mutations below...
                if (operationName === 'get' && hasIdVariable) {
                    extraTypes.push(`export type ${optionsTypeName} = Omit<${o.operationVariablesTypes}, 'id'>;`);
                    parameters = `idOrOptions: string | ${o.operationVariablesTypes}, options?: ${optionsTypeName}`;
                    mergeParamsIntoVariables = `const variables = typeof idOrOptions === 'string' ? { ...options, id: idOrOptions } : { ...options, ...idOrOptions }`;
                }
            }
            if (o.operationType === 'Mutation') {
                ...
                if (hasInputVariable && hasIdVariable && !isInvokeContract) {
                    ...
                    if (isStringInput) {
                        ...
                        const idProperty = hasUniqueKeys ? '_id?: string' : '_id: string';
                        extraTypes.push(`export type ${inputTypeName} = ${o.operationVariablesTypes}['input'] & { ${idProperty} };`);
                        // For PUT with ACLs, aclInput is required SO make options required (that's where aclInput goes)
                        const optionsKey = operationName === 'put' && isAclEnabled ? 'options:' : 'options?:';
                        // Extract _id from the new input type and pass it to the id variable
                        parameters = `input: ${inputTypeName}, ${optionsKey} ${optionsTypeName}`;
                        mergeParamsIntoVariables = `const { _id, ...rest } = input;\n  const variables = { ...options, id: _id, input: rest }`;
                        // For updates with unique keys we need to figure out what key arg to use
                        //   NOTE: for PUT, we use _id arg if they pass it in otherwise we always assume its an UPSERT and never use any unique key arguments!
                        //     If we did pass a unique key arg, back-end tries to find the object first _and then_ PUT it, will fail if object doesn't exist
                        if (operationName === 'update' && hasUniqueKeys) {
                            // - if they pass in _id, we'll just use that
                            // - if they pass in more one unique key arg(s), we'll use the first one we find
                            // - add an option to let them specify the key arg to use??? not for now
                            mergeParamsIntoVariables = `${mergeParamsIntoVariables}
  if (!variables.id) {
    // If _id is not passed in, try to find one of the unique key args and use that
    const uniqueKey = Object.keys(rest).find((key) => ${JSON.stringify(uniqueKeys)}.includes(key))
    if (uniqueKey) {
      variables[uniqueKey] = rest[uniqueKey]
    }
  }`;
                        }
                    }
                ...
                if ((hasIdVariable && !hasInputVariable) || isInvokeContract) {
                    // product.delete(id, options) OR product.delete(options) - both APIs are supported for entities without unique keys
                    // NOTE: see comment above about GET queries - product.get(id, options) - same applies here!!! 
...
function uniqueKeysToParams(uniqueKeys, entityJson) {
    // Should never fail to find type, but fall back to 'any' just in case
    return uniqueKeys.map((key) => `${key}?: ${jsonSchemaToTypescriptType?.[entityJson?.items?.properties?.[key]?.type] ?? 'any'}`).join(', ');
}
